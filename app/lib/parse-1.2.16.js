		 ...
		 var eq = function(a, b, aStack, bStack) {
					...
					switch (className) {
							 ...
							 case '[object String]':
										...
										return a == String(b);
							 case '[object Number]':
										// `NaN`s are equivalent, but non-reflexive. An `egal` comparison is performed
										// for
										// other numeric values.
										return a != +a ? b != +b : (a == 0 ? 1 / a == 1 / b : a == +b);
							 case '[object Date]':
							 case '[object Boolean]':
										// Coerce dates and booleans to numeric primitive values. Dates are compared by
										// their
...
( function(root) {
					 ...
					 */
					Parse.initialize = function(applicationId, javaScriptKey, restApiKey, masterKey) {
							 if (masterKey) {
										throw "Parse.initialize() was passed a Master Key, which is only " + "allowed from within Node.js.";
							 }
							 Parse._initialize(applicationId, javaScriptKey, restApiKey);
					};
					 ...
					 */
					Parse._initialize = function(applicationId, javaScriptKey, restApiKey, masterKey) {
					...
					Parse._ajax = function(method, url, data, success, error) {
							 ...
							 if (isAndroid) {
										xhr.onerror = function() {
												 ...
												 Ti.API.info('Android onerror response :' + JSON.stringify(response));
												 promise.reject(xhr);
										};
										if (method == "GET") {
												 var newUrl = "";
												 var jSONobj = JSON.parse(data);
												 var paramMap = jSONobj || {};
												 for (var a in paramMap) {
															newUrl += Titanium.Network.encodeURIComponent(a) + '=' + Titanium.Network.encodeURIComponent(paramMap[a]) + '&';
												 }
												 url = url + "?" + newUrl;
												 ...
												 xhr.setRequestHeader("X-Parse-REST-API-Key", Parse.restApiKey);
...
( function(root) {
					...
					var provider = {
							 authenticate : function(options) {
										...
										/*FB.login(function(response) {
										 ...
										 expiration_date : new Date(response.authResponse.expiresIn * 1000 + (new
										 Date()).getTime()).toJSON()
										 });
										 }
										 } else {
					...
					Parse.FacebookUtils = {
							 ...
							 logIn : function() {
										...
										var utilityParse = require("/utility");
										Ti.API.info('Parse Facebook is loggedIn :' + Alloy.Globals.fb.loggedIn);
										...
										// Either way I resolved the problem, calling _handleSaveResult(true) on the
					...
					Parse.TwitterUtils = {
							 ...
							 logIn : function(twitterKeys, options) {
										var utilityParse = require("/utility");
										var Twitter = require('/twitter/twitter').Twitter;
										var client = Twitter({
										...
										client.addEventListener('login', function(e) {
												 ...
												 if (e.success) {
															// var utilityParse = require("/utility");
															client.request("1.1/account/update_profile.json", [], "POST", function(response) {
																	 if (response.success) {
																				var jsonResultData = JSON.parse(response.result.text);
																				// ..twitter oauth data format
																				var authData = {
																						 "twitter" : {
																									"id" : jsonResultData.id_str,
																									"screen_name" : jsonResultData.screen_name,
																									"consumer_key" : twitterKeys.consumerKey,
																									"consumer_secret" : twitterKeys.consumerSecret,
																									"auth_token" : Ti.App.Properties.getString("twitter_oauthToken"),
																									"auth_token_secret" : Ti.App.Properties.getString("twitter_oauthTokenSecret")
																						 }
																				};
																				var twitterUser = new Parse.User();
																				twitterUser.save({
																						 "authData" : authData
																				}).then(function(_user) {
																						 // force the user to become current
																						 _user._handleSaveResult(true);
																						 //<-- this is the evil method I called
																						 var userflName = jsonResultData.name;
																						 var userFirstLastname = userflName.split(" ");

																						 if (!_user.existed()) {
																									function downLoadImage(imageUrl, imageOptions) {
																											 var xhr = Titanium.Network.createHTTPClient();
																											 xhr.open("GET", imageUrl);
																											 xhr.onload = function() {
																														imageOptions.success(xhr.responseData);
																											 };
																											 xhr.onerror = function(e) {
																														imageOptions.error(e);
																											 };
																											 xhr.send();
																									}

																									(function validUserInfo() {
																											 if (jsonResultData.hasOwnProperty("profile_image_url_https")) {

																														downLoadImage(jsonResultData.profile_image_url_https, {

																																 success : function(responseData) {

																																			var twitterPicResult = utilityParse.saveImageToFile(responseData);
																																			...
																																			if (profileImageFile.exists()) {
																																					 ...
																																					 parseFile.save().then(function(obj) {
																																								...
																																								var userInfo = {
																																										 username : jsonResultData.screen_name,
																																										 password : jsonResultData.id_str,
																																										 ...
																																										 isPinPrivate : false,
																																										 fbUserId : "",
																																										 twitterUserId : jsonResultData.id_str
																																								};
																																								return _user.save(userInfo).then(function(userResData) {
																																										 if (userResData) {
																																													Alloy.Globals.UserId = userResData.id;
																																													var userRelation = Parse.Object.extend("userRelation");
																																													var newUserRelation = new userRelation();
																																													newUserRelation.save({
																																															 userObjectId : userResData.id,
																																															 following : [],
																																															 followers : []
																																													}, {
																																															 success : function(sucresult) {
																																																		Ti.API.info('User Relation Created Success :' + JSON.stringify(sucresult));
																																															 },
																																															 error : function(error) {
																																																		Ti.API.info('User Relation Created error :' + JSON.stringify(error));
																																																		options.error(error);
																																															 }
																																													});
																																													options.success(userResData);
																																										 }

																																								}, function(tt) {
																																										 Ti.API.info('Error Save  with profile image :' + JSON.stringify(tt));
																																										 
																																										 //here Parse enter random name in username filed
																																										 options.error(tt);
																																								});
																																					 }, function(error) {
																																								Ti.API.info("Unable to upload image.Please Try  again.");
																																								var userInfo = {
																																										 username : jsonResultData.screen_name,
																																										 password : jsonResultData.id_str,
																																										 firstName : userFirstLastname[0],
																																								...
																																								return _user.save(userInfo).then(function(userResData) {
																																										 Ti.API.info('@@@@########@@@@@@@  Save then without profile image' + JSON.stringify(userResData));
																																										 if (userResData) {
																																													var userRelation = Parse.Object.extend("userRelation");
																																													var newUserRelation = new userRelation();
																																													Alloy.Globals.UserId = userResData.id;
																									...
																									//Do login
																									Parse.User.logIn(jsonResultData.screen_name, jsonResultData.id_str, {
																											 success : function(newuser) {
																														Ti.API.info('Twitter Already user login Success');
																														options.success(newuser);
																											 },
																											 error : function(newuser, error) {
																														Ti.API.info('Twitter Already user login Error');
																														options.error(error);
																											 }
																									});
																						 ...
																						 // Ti.API.info('For  ' + JSON.stringify(userData));
																				}, function(error) {
																						 // Ti.API.info('Twitter login Error:' + JSON.stringify(error));
																						 options.error(error);
																				});
