...
( function(root) {
		...
		Parse._request = function(options) {
			...
			if (route !== "batch" && route !== "classes" && route !== "events" && route !== "files" && route !== "functions" && route !== "login" && route !== "push" && route !== "requestPasswordReset" && route !== "users" && route !== "jobs") {
...
( function(root) {
		...
		var readAsync = function(file, type) {
			...
			if ( typeof (FileReader) === "undefined") {
				return Parse.Promise.error(new Parse.Error(-1, "Attempted to use a FileReader on an unsupported browser."));
			}
			...
			reader.onloadend = function() {
				...
				if (!matches) {
					promise.reject(new Parse.Error(-1, "Unable to interpret data URL: " + dataURL));
					return;
				}
